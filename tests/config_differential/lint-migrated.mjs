// Runs core's canonical config lint over config/RedEclipseHeadTracking.ini, the committed
// file, and over every distinct CameraUnlock.ini the differential test migrated (the folder it
...
const committed = path.join(repo, "config", "RedEclipseHeadTracking.ini");
for (const problem of lintCanonicalConfig(fs.readFileSync(committed), options)) {
  failures.push(`config/RedEclipseHeadTracking.ini: ${problem}`);
}
